...
public class TreinoDAO {
	...
	public Treino inserir(Treino novoTreino) {
		...
		String sql =  " INSERT INTO TREINO (ID_CLIENTE, ID_PROFISSIONAL, DT_CADASTRO, DT_TERMINO, "
					+ " NIVEL, TREINO) "
				    + " VALUES (?,?,?,?,?,?) ";
		...
		try {
			...
			query.setString(4, novoTreino.getDtCadastro());
			query.setString(5, novoTreino.getDtTermino());
			query.setInt(6, novoTreino.getNivelTreino().getValor());
			query.setString(7, novoTreino.getTreino());
			query.execute();
			...
			System.out.println("Erro ao inserir endereÃ§o. "
					+ "\nCausa: " + e.getMessage());
		}finally {
			//Fechar a conexão
	...
	private Treino converterDeResultSetParaEntidade(ResultSet resultado) throws SQLException {
		...
		treinoConsultado.setCliente(usuario.consultarPorId(resultado.getInt("ID_CLIENTE")));
		treinoConsultado.setProfissional(usuario.consultarPorId(resultado.getInt("ID_PROFISSIONAL")));
